...
public class JDBCSelect {
	...
	public static void main(String[] args) {
		//JDBC 연동
		// MySQL 을 연동할 것임을 알리기 위해 forName 내부에 MySQL용 연동구문을 적습니다.
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");//접속 타겟이 MySQL 8버전임을 명시
			// 커넥션 객체는 연결 여부를 확인합니다.
			// 입력요소는 접속주소, MySQL계정명, MySQL 비밀번호 순으로 입력합니다.(목적지인 MySQL 서버주소를 적습니다.)
			...
			// 쿼리문은 Statement라는 변수에 저장해서 날려줘야합니다.
			Statement stmt = con.createStatement();
			//.execueQuery()내부에 실행할 쿼리문을 문자열로 작성합니다.
			// SELECT문을 실행할 때 executQuerySet 이라는 자료로 받을 수 있습니다. 
			...
			// 해당 컬럼의 자료를 반환합니다. 
			// 몇 줄이 오더라도 정확하게 존재하는 row개수만큼만 도는 반복문
			while(rs.next()) {
				...
				//rs.next();
				//System.out.println(rs.getString(1)); //숫자 대신 "user_id(컬럼)을 써도된다.
				//System.out.println(rs.getString(2));
			    //System.out.println(rs.getString(3));
				//System.out.println(rs.getString(4));
				//System.out.println("-------------");
