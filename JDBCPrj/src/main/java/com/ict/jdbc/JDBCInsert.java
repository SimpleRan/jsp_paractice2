...
public class JDBCInsert {
	...
	public static void main(String[] args) {
		...
		// SELECT 구문을 제외한 나머지 INSERT, DELET, UPDATE 구문은
		// 조회목적으로 날리는 데이터가 없기 때문에 ResultSet을 필요로 하지 않습니다.
		// 실행이 된 다음에 MySQL Workbench를 켜서 데이터 입력 여부만 확인해주시거나
		// 혹은 SELECTQ1에 입력한 아이디를 넣어 확인해주시면 됩니다.
		
		// 쿼리문 실행시 SELECT 구문은 executeQuery("쿼리문"); 을 호출했지만
		// SELECT가 아닌 구문은 executeUpdate("쿼리문");dmf eotls ghcnfgkau 
		...
		Scanner scan = new Scanner(System.in);
			...
			try {  
				// 1. try 블럭 생성 및 Connection 생성
				Class.forName("com.mysql.cj.jdbc.Driver"); // 접속 타겟이 MySQL8버전인 경우 좌측과 같이 작성	
				Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/jdbcprac2?serverTimezone=UTC", "root","mysql"); 
